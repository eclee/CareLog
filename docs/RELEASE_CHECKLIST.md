...
- [ ] 驗證只有內建 `admin` 不可刪除，後續新增管理者可由另一位管理者刪除
- [ ] 驗證長輩性別、血型、身高、病史、過敏、就醫與緊急聯絡欄位
- [ ] 驗證兩位長輩可有不同健康數據預設值，前台切換後不交叉帶入
...
- [ ] 驗證五種語言與手機窄螢幕導覽列
- [ ] 驗證新增任何角色時 PIN 與密碼皆必填，舊帳號缺少憑證時必須補齊
- [ ] 驗證只有內建 `admin` 的角色固定，其他帳號可切換管理者／家屬／照顧者
- [ ] 驗證含 `U+00A0`、`U+202F` 或零寬字元的 Gmail 應用程式密碼可正常正規化與寄送
- [ ] 驗證照顧者登入後立即套用帳號預設語言
- [ ] 驗證未填報提醒四個時段可分別啟用／關閉
- [ ] 以數字型條件實測照片、異常、操作紀錄、長輩及參數頁篩選，不得出現 500

## 升級與資料

- [ ] 以 1.1.0、1.2.0、1.3.0 與 1.3.1 測試資料庫執行／驗證升級與直接沿用
- [ ] 執行 `flask --app app check-db` 並確認資料庫實際路徑與結構版本
- [ ] 連續執行兩次升級命令，第二次不重複修改
...
- [ ] 初始管理者密碼 `care1234` 與 PIN `1234` 已標示必須立即更換
...
- [ ] `VERSION` 為 `1.3.2`
- [ ] `pyproject.toml` 版本為 `1.3.2`
- [ ] Docker image tag 為 `carelog:1.3.2`
- [ ] `CHANGELOG.md` 有 `1.3.2` 與發布日期
- [ ] README 的本機連結均可開啟
- [ ] `README.md`、`README.en.md`、架構、建置及升級文件同步
- [ ] `LICENSE`、`CONTRIBUTING.md`、`CODE_OF_CONDUCT.md` 與 `DISCLAIMER.md` 已納入
- [ ] 建立 `v1.3.2` tag，Release 說明引用 `CHANGELOG.md`
