...
`login_required(*roles)` 先確認 session 使用者仍存在、啟用且未被刪除，再核對角色。CareLog 目前沒有使用者－長輩授權表，因此啟用中的使用者可依角色查看所有啟用長輩。
...
| 模型 | 主要內容 |
|---|---|
| `User` | 帳號、角色、密碼雜湊／PIN、語言、啟用及刪除時間 |
...
1. 禁止刪除目前登入者；
2. 停用帳號並記錄 `deleted_at`；
3. 清除密碼雜湊或 PIN，阻止再次登入；
4. 從一般使用者管理清單隱藏；
5. 保留 `User` 資料列、照顧紀錄的 `created_by` 與歷史操作身分；
6. 新增帳號刪除稽核事件。
...
同日多筆喝水先加總。「有紀錄日平均／最低／最高」只採計有紀錄日期；另提供「全期間日均」，以期間總量除以完整天數。如此能區分未填資料與確定 0 ml。

### 用藥

目前儀表板的給藥率以「已填報用藥項目」為分母。真正的應服藥遵從率仍需用藥計畫生效日、停用日與版本歷史，列為後續擴充。

## 資料庫升級

`services/schema.py` 提供依賴較少、可重複執行的 SQLite 相容性升級：
